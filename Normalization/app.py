...
def preprocess(
    ...
    # do_QC : whatever we filter out "outlier" cells with high total counts and number of expressed genes.

    adata.uns['prepro'] = {}

    if 'blank_genes' in adata.obsm:
        print('Filtering on blank genes...')
        noise_floor = adata.obsm['blank_genes'].max(
            axis=1)  # We assume that each "blank gene" is a n-bit barcode error and not a bit flip error.
...
def MAD_threshold(variable, ndevs=5):
...
def run(**kwargs):
    ...
    scale_max = kwargs.get('clip_value')
